...
const Hero: React.FC<HeroProps> = ({ lang, t, isLoggedIn, onCta, onLoginClick }) => {
  return (
    <section className="relative min-h-[80vh] flex items-center overflow-hidden bg-white dark:bg-zinc-950 py-10 px-5 transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-10 lg:gap-20 relative z-10 w-full">
        <div className="flex-[1.2] text-center lg:text-left space-y-6">
          ...
          <h1 className={`text-4xl md:text-7xl lg:text-8xl font-black leading-[0.95] text-zinc-900 dark:text-zinc-100 tracking-tightest ${lang === 'en' ? 'font-english uppercase' : 'font-bengali'}`}>
            {lang === 'en' ? (
              <>UNLEASH <span className="text-red-600 italic">LIMITLESS</span> SPEED.</>
            ) : (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 pt-4">
            <button onClick={onCta} className="w-full sm:w-auto px-8 py-4 bg-red-600 text-white font-black rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-zinc-900 dark:hover:bg-zinc-100 dark:hover:text-zinc-900 shadow-xl shadow-red-600/10">
              <ShoppingBag size={18} />
            ...
            {!isLoggedIn && (
              <button onClick={onLoginClick} className="w-full sm:w-auto px-8 py-4 bg-zinc-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 font-black rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-zinc-200 dark:hover:bg-zinc-800">
                <UserCircle size={18} />
        ...
        <div className="flex-1 w-full max-w-sm lg:max-w-md relative group">
          <div className="relative aspect-square overflow-hidden rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 shadow-2xl">
            <img src="https://images.unsplash.com/photo-1544197150-b99a580bb7a8?auto=format&fit=crop&q=80&w=800" alt="Fiber" className="w-full h-full object-cover dark:grayscale dark:brightness-[0.5] transition-all duration-700 group-hover:scale-105" />
